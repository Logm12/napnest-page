...
export default function Showcase() {
  ...
  return (
    <section id="showcase-section" className="bg-slate-950 py-20 relative overflow-hidden border-b border-slate-900">
      {/* Dynamic Background Glow */}
      <div className="absolute top-10 left-1/3 w-80 h-80 bg-blue-500/5 rounded-full blur-3xl pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
          {filteredImages.map((img, idx) => (
            <div
              key={idx}
              className="bg-slate-900/80 border border-slate-850 rounded-2xl overflow-hidden shadow-xl transition-all duration-500 hover:border-slate-750 group flex flex-col justify-between"
            >
              {/* Image Box */}
              <div className="relative h-72 w-full bg-slate-950 overflow-hidden">
                ...
                <div className="absolute inset-0 bg-slate-950/45 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center pointer-events-none">
                  <div className="w-10 h-10 rounded-full bg-blue-500 text-white flex items-center justify-center shadow-lg transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                    <Eye className="w-5 h-5" />
                  </div>
                </div>

                <div className="absolute top-3 left-3 bg-slate-950/80 backdrop-blur-md px-2.5 py-1 rounded-md text-[9px] font-mono text-slate-400 border border-slate-800">
              ...
              <div className="p-5 flex-1 flex flex-col justify-between text-left">
                ...
                <div className="mt-4 pt-4 border-t border-slate-800/80 space-y-1.5 font-mono text-[10px] text-slate-500">
